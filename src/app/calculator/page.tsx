...
export default function CalculatorPage() {
  ...
  const results = [
    { name: 'Fixné náklady (Domácnosť, účty)', percent: split.fixed_costs, color: 'blue' },
    { name: 'Investície (ETF, Akcie)', percent: split.investments, color: 'emerald' },
    { name: 'Krátkodobé sporenie (Rezerva)', percent: split.savings, color: 'amber' },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500">Rozdelenie príjmu podľa nastavených percent.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Mesačný príjem (€)</label>
            <input 
              type="number"
          ...
          <div className="space-y-4">
            <label className="text-sm font-semibold text-slate-500 uppercase tracking-wider block">Rozdelenie (%)</label>
            ...
            <div className={`text-xs font-bold text-right ${totalPercent === 100 ? 'text-emerald-500' : 'text-rose-500'}`}>
              Spolu: {totalPercent}% {totalPercent !== 100 && '(musí byť 100%)'}
            </div>
          </div>
        ...
        <div className="space-y-4">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl bg-${item.color}-50 dark:bg-${item.color}-950 flex items-center justify-center text-${item.color}-600 dark:text-${item.color}-400 font-bold`}>
                    {item.percent}%
                  </div>
                  <div>
                    <p className="text-sm text-slate-500">{item.name}</p>
                    <h4 className="text-xl font-bold">{formatCurrency(amount)}</h4>
                  </div>
                </div>
          ...
          <div className="bg-blue-600 p-6 rounded-2xl text-white shadow-lg shadow-blue-200 dark:shadow-none">
            ...
            <p className="text-blue-100 text-sm leading-relaxed">
              Pravidlo 50/30/20 je dobrý štart, ale tvoje nastavenie ({split.fixed_costs}/{split.investments + split.savings}/{split.fun}) je ešte ambicióznejšie pre budovanie bohatstva!
            </p>
          </div>
        </div>
      </div>
